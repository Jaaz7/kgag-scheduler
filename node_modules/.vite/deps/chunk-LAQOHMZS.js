...
var _ = Object.entries(E).reduce((e, [a, o]) => (e[o] || (e[o] = []), e[o].push(a), e), {});
...
var S = ({ __devtools: e, port: a, children: o }) => {
  let [s, r] = import_react.default.useState({ __devtools: e ?? false, port: a ?? 5001, url: "localhost", secure: false, ws: null, devtoolsUrl: "http://localhost:5001" }), [u, l] = import_react.default.useState(null);
  import_react.default.useEffect(() => {
    let d = null, n = new WebSocket(`${s.secure ? "wss" : "ws"}://localhost:${s.port}`), c = T(n, "devtools:handshake", (D) => {
      r((L) => ({ ...L, devtoolsUrl: D.url })), c();
    });
    return n.addEventListener("open", () => {
      s.__devtools || (d = setTimeout(() => {
        O(n, "devtools:init", { url: window.location.origin });
      }, 300));
    }), l(n), () => {
      c(), d && clearTimeout(d), n.readyState === WebSocket.CONNECTING ? n.addEventListener("open", () => {
        n.close(1e3, window.location.origin);
      }) : n.close(1e3, window.location.origin);
    };
  }, []);
  let p = import_react.default.useMemo(() => ({ ...s, ws: u }), [s, u]);
  return import_react.default.createElement(v.Provider, { value: p }, o);
};
...
//# sourceMappingURL=chunk-LAQOHMZS.js.map
